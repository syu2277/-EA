# FX_Combined_Portfolio_Currency_V1_2 仕様書 / Codexレビュー用
...
- 通貨版: `FX_Combined_Portfolio_Currency_V1_2.mq5`
...
V1.2では、初期OFFのSell専用スナイパー側も今後ON検証できるように、Gold用の固定pips値を通貨用へ変更した。
...
通貨版V1.2でも初期OFF。ただし、将来ON検証できるようにpips・ロット・SL関連の初期値は通貨用へ修正済み。
...
## V1.2追加仕様：EA② 最大保有バー数決済

通貨版は短期回転型として検証するため、EA②の通常Buy/Sellポジションに最大保有バー数決済を追加する。

- `InpUseMaxHoldBars = true` を初期値とする。
- `InpMaxHoldBars = 24` を初期値とする。M15では約6時間相当。
- 対象はEA②の `InpMagicNumber` のポジションのみ。EA① DowFib とSell専用スナイパーには干渉しない。
- 複数ポジションがある場合は、EA②サイクル内の最古ポジションの `POSITION_TIME` を基準にする。
- `iBarShift(_Symbol, _Period, earliestOpenTime, false)` が `InpMaxHoldBars` 以上になったら、EA②ポジションを成行決済する。
- 決済後、EA②ポジションがなくなればサイクル状態を `ResetCycleIfFlat()` で初期化する。

初期検証では、TP/SL/建値/半利確に到達しないまま長時間ヨコヨコ化したポジションを整理する目的で使用する。
...
1. `InpUseMaxHoldBars` / `InpMaxHoldBars` の入力パラメータがEA②側に追加されているか。
2. `ManageMaxHoldBars()` がEA②の `InpMagicNumber` だけを対象にしているか。
3. 最大保有バー数到達時にEA①やSell専用スナイパーのポジションを誤決済しないか。
4. 最大保有決済後に `ResetCycleIfFlat()` によりEA②サイクル状態が初期化されるか。
5. M15で `InpMaxHoldBars=24` が約6時間相当として機能するか。
